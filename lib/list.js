 ...
 * @param {Number} index optional
 * @returns {Object} the replace item
 * @api public
 */
 ...
 * with `newitem`
 *
 *    ls.replaceIf('newVal', function(){})
 *    ls.replaceIf('newVal', {yo:'joe'}, function(){})
 *
 * @param {Function} iterator
 * @param {Object} context optional
 * @param {Object} newitem
...
List.prototype.replaceIf = function replaceIf(newitem, context, iterator) {
  if (arguments.length === 2) {
 ...
 * with `newitem`
 *
 *    ls.replaceAll('newVal', function(){})
 *    ls.replaceAll('newVal', {yo:'joe'}, function(){})
 *
 * @param {Object} newitem
 * @param {Object} context optional
 * @param {Function} iterator
 * @returns {List} list of items replaced
 * @api public
 */

List.prototype.replaceAll = function replaceAll(newitem, context, iterator) {
  if (arguments.length === 2) {
    iterator = arguments[1];
    context  = null;
  }
...
/**
 * Returns a copy for the list with all occurences of `undefined`,
...
List.prototype.group = function group(context, iterator) {
  ...
  var group = {};
  this.forEach(this, function(val, i, ls) {
...
List.prototype.sort = function sort(comparer) {
  if (this.length <= 1) return new List;
  var ls = this.clone();
  if (comparer != null)
    return _sort.call(ls, comparer);

  var isnumeric = ls.every(function(val) {
  ...
  if (isnumeric) {
    return _sort.call(ls, function(a, b) {
      return a - b;
    });
  }
  else
    return _sort.call(ls);
}
