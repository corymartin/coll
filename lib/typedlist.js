...
Object.defineProperty(TypedList.prototype, '_checkType', {
  value: function(obj) {
    ...
    else if (obj == null)         return; // Allow `null`s
    else {
      isType = typeof this.type === 'function'
...
TypedList.prototype.unshift = function() {
  if (!util.isTypeEvery(arguments, this.type)) util.throwTypeError(this.type);
...
TypedList.prototype.splice = function() {
  if (arguments.length >= 3 && !util.isTypeEvery(arguments, this.type, 2))
...
TypedList.prototype.insertRange = function(index, iterable) {
  ...
  if (!util.isTypeEvery(iterable, this.type)) util.throwTypeError(this.type);
  var args = [index, 0];
 ...
 * If the optional iterator function is passed, the value returned from it
...
TypedList.prototype.max = function(iterator, context) {
  ...
  var max = this.first();
  var _a = iterator.call(context, max);
  var _b;
  return this.reduce(function(a, b) {
    _b = iterator.call(context, b);
    return max = +_a > +_b
      ? a
      : (_a = _b, b);
  });
...
TypedList.prototype.min = function(iterator, context) {
  if (!this.length) return;
  if (iterator == null) {
    return this.reduce(function(a, b) {
      return a = +a < +b ? a : b;
  ...
  var min = this.first();
  var _a = iterator.call(context, min);
  var _b;
  return this.reduce(function(a, b) {
    _b = iterator.call(context, b);
    return min = +_a < +_b
